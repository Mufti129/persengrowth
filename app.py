...
# ==============================================================================
# 1. PENGAMBILAN DATA LIVE DARI GOOGLE SHEETS VIA LINK CSV EXPORT
# ==============================================================================
@st.cache_data
def load_data():
    try:
        # Mengonversi link Google Sheets Anda ke format ekspor CSV langsung
        sheet_url = "https://docs.google.com/spreadsheets/d/12tbxOsTfAAuCA3LRNOOQ3x9anvy2JYU_n7ESEVXjCdY/export?format=csv&gid=1513537728"
        df = pd.read_csv(sheet_url)
        
        # Standarisasi kolom Persen Growth jika ada
...
if df_clean is not None:
    ...
    if 'Growth' in df_clean.columns:
        heat_data = [[row['latitude_cabang'], row['longitude_cabang'], row['Growth']] for i, row in df_clean.iterrows() if pd.notna(row['latitude_cabang']) and pd.notna(row['longitude_cabang'])]
    else:
        heat_data = [[row['latitude_cabang'], row['longitude_cabang'], row['Persen Growth']] for i, row in df_clean.iterrows() if pd.notna(row['latitude_cabang']) and pd.notna(row['longitude_cabang'])]
    ...
    for i, row in df_clean.iterrows():
        if pd.isna(row.get('latitude_cabang')) or pd.isna(row.get('longitude_cabang')):
            continue

        raw_growth = row.get('Persen Growth', 0)
        display_growth = raw_growth * 100 if raw_growth <= 1.0 else raw_growth

        # Proteksi parsing nilai nominal omset jika ada data kosong/string kosong
        omset_jan = int(row.get('Omzet Jan25', 0)) if pd.notna(row.get('Omzet Jan25', 0)) else 0
        omset_mei = int(row.get('Omzet Mei26', 0)) if pd.notna(row.get('Omzet Mei26', 0)) else 0
        ...
        popup_content = (
            ...
            f"  <b>Omset Jan25:</b> Rp {omset_jan:,}<br>"
            f"  <b>Omset Mei26:</b> Rp {omset_mei:,}<br>"
            f"  <b>Persen Growth:</b> {display_growth:.2f}%<br>"
            f"  <hr style='margin:5px 0; border:0; border-top:1px solid #eee;'>"
